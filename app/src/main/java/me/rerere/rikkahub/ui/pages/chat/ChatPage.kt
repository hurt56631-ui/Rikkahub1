        ...
        Scaffold(
            ...
            bottomBar = {
                ...
                ChatInput(
                    ...
                    temporaryChat = conversation.isTemporary,
                    onToggleTemporaryChat = {
                        vm.toggleTemporaryConversation { enabled, error ->
                            if (error != null) {
                                toaster.show(error, type = ToastType.Error)
                            } else {
                                toaster.show(
                                    if (enabled) "临时会话已开启：退出后不保存到本地历史" else "临时会话已关闭：当前会话会正常保存",
                    ...
                    onSendClick = {
                        ...
                        if (inputState.isEditing()) {
                            vm.handleMessageEdit(
                                parts = inputState.getContentsForSend(),
                                messageId = inputState.editingMessage!!,
                            )
                        } else {
                            vm.handleMessageSend(inputState.getContentsForSend())
                            scope.launch {
                    ...
                    onLongSendClick = {
                        if (inputState.isEditing()) {
                            ...
                            vm.handleMessageSend(content = inputState.getContentsForSend(), answer = false)
                            scope.launch {
                                chatListState.requestScrollToItem(conversation.currentMessages.size + 5)
                            }
                        }
