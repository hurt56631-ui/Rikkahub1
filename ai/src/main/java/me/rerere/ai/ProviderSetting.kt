...
sealed class ProviderSetting {
    ...
    /**
     * Gemini Web provider (reverse engineered gemini.google.com protocol).
     *
     * Authentication is intentionally not serialized here. The provider reuses the WebView
     * cookie jar at runtime so Google session cookies never enter RikkaHub provider exports,
     * WebDAV backups, or S3 backups.
     */
    @Serializable
    @SerialName("gemini_web")
    data class GeminiWeb(
        override var id: Uuid = Uuid.random(),
        override var enabled: Boolean = true,
        override var name: String = "Gemini Web",
        override var models: List<Model> = emptyList(),
        override val balanceOption: BalanceOption = BalanceOption(),
        @Transient override val builtIn: Boolean = false,
        @Transient override val description: @Composable (() -> Unit) = {},
        @Transient override val shortDescription: @Composable (() -> Unit) = {},
        /** Google account index used by gemini.google.com (/u/{index}/app). */
        val authUser: String = "0",
        /** Use Gemini Web native temporary-chat flag so requests are not added to Google history. */
        val temporaryChatOnGoogle: Boolean = true,
    ) : ProviderSetting() {
